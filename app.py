...
with tab_roadmap:
    ...
    with col_s3:
        if cat_filter != "All":
            work_item_options = ["All"] + sorted(categories.get(cat_filter, {}).keys())
        else:
            all_items = sorted(set(item for items in categories.values() for item in items.keys()))
            work_item_options = ["All"] + all_items
        work_filter = st.selectbox("Filter by Work Item", work_item_options)

    filtered = df_roadmap.copy()
    if cat_filter != "All":
        filtered = filtered[filtered.iloc[:, 1] == cat_filter]
    if work_filter != "All":
        filtered = filtered[filtered.iloc[:, 2] == work_filter]
    if search:
        mask = filtered.apply(lambda row: row.astype(str).str.contains(search, case=False).any(), axis=1)
        filtered = filtered[mask]

    # Convert to string to avoid Arrow serialization errors with mixed types
    st.dataframe(filtered.astype(str).replace("None", ""), use_container_width=True, height=420)
